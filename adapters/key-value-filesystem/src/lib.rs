//! Filesystem realization of `KeyValueStore`.
...
use onoal_fabric_resource_key_value::{KeyValueError, KeyValueStore, KeyValueStoreRealization};
...
fabric::adapter! {
    pub FilesystemKeyValueAdapter for resource KeyValueStore implements KeyValueStoreRealization {
        schema: provisional;
        realization: "1.0.0";
            ...
            fn put(&self, key: Vec<u8>, value: Vec<u8>) -> Result<(), KeyValueError> {
                ...
                if self.config.fsync {
                    fs::OpenOptions::new()
                        .read(true)
                        .open(path)
                        .map_err(|_| KeyValueError::Unavailable)?
                        .sync_all()
                        .map_err(|_| KeyValueError::Unavailable)?;
                }
                Ok(())
...
mod tests {
    ...
    #[test]
    fn encoded_keys_stay_beneath_the_configured_root() {
        let root = PathBuf::from("/tmp/onoal-key-value-root");
        let path = key_path(&root, &FileLayout::Sharded { depth: 2 }, b"../../outside")
