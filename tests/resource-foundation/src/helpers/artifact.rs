...
pub(crate) fn create_artifact_fixture(layout: &HarnessLayout) -> ArtifactFixture {
    ...
    std::fs::write(&entry, workload_source()).expect("write resource foundation workload");

    let artifact_file = artifacts_root.join("artifact.bin");
    std::fs::write(&artifact_file, b"resource-foundation-artifact").expect("write artifact bytes");
    let sha256 = Sha256::digest(std::fs::read(&artifact_file).expect("read artifact bytes"));

    ArtifactFixture {
        artifact: WorkloadArtifact {
            reference: "fabric://test/resource-foundation".to_owned(),
            sha256: sha256.into(),
        },
...
async function readState(bindings) {
  ...
  await dbExecute(
    { baseUrl: dbBaseUrl },
    "CREATE TABLE IF NOT EXISTS e9_state (key TEXT PRIMARY KEY, value TEXT NOT NULL);",
  );
  const rows = await dbQuery(
    { baseUrl: dbBaseUrl },
    "SELECT value FROM e9_state WHERE key = 'db' LIMIT 1;",
  );
  const dbValue = rows.rows?.[0]?.[0] ?? null;
  const kvValue = await kvGet(bindings.CACHE, "e9-state");
  return { dbValue, dbBaseUrl, kvValue, secretValue };
}

async function serviceCheck() {
  const serviceBaseUrl = readOptionalEnv("SERVICE_BASE_URL");
  if (!serviceBaseUrl) {
    throw new Error("missing SERVICE_BASE_URL projection");
  }
  const response = await globalThis.fetch(`${serviceBaseUrl}/status?check=1`, {
    method: "GET",
    headers: { "x-e9-proof": "yes" },
  });
  return {
    status: response.status,
    header: response.headers.get("x-service-proof"),
...
export async function fetch(request, env) {
  ...
  if (request.method === "GET" && url.pathname === "/service-check") {
    return Response.json(await serviceCheck());
  }
  ...
  if (request.method === "POST" && url.pathname === "/state") {
    ...
    await dbExecute(
      bindings.DB,
      `INSERT INTO e9_state(key, value) VALUES ('db', '${payload.dbValue}')
       ON CONFLICT(key) DO UPDATE SET value = excluded.value;`,
    );
    await kvPut(bindings.CACHE, "e9-state", payload.kvValue);
